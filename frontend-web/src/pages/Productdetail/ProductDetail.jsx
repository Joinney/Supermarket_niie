...
export default function ProductDetail() {
        ...
        if (productData && productData.ma_san_pham) {
          ...
          if (targetVariant) {
            ...
            if (!variantId || variantId !== targetVariant.ma_bien_the) {
              ...
              navigate(`/${currentCountry}/product/${cSlug}/${id}/${targetVariant.ma_bien_the}`, { replace: true });
            }
          }
        } else {
          setProduct(null);
  ...
  const handleAttributeSelect = (key, value) => {
    ...
    if (matchedVariant) {
      ...
      navigate(`/${country}/product/${category_slug}/${id}/${matchedVariant.ma_bien_the}`, { replace: true });
    } else {
  ...
  const displayCategoryName = location.state?.categoryName || product?.ten_dm_con || product?.ten_danh_muc;
  const displayCategorySlug = location.state?.categorySlug || product?.slug_danh_muc || product?.ma_dm_con;
          ...
          <div className="lg:col-span-6 xl:col-span-5 text-left space-y-4 lg:space-y-5 lg:sticky lg:top-4 px-1">
            ...
            <div className="flex items-baseline gap-3 border-b border-slate-100 pb-3 lg:pb-4">
              {selectedVariant ? (
                ...
                <span className="text-xl lg:text-2xl font-black text-rose-500 uppercase tracking-wide italic">Tạm hết hàng</span>
              )}
            </div>
              ...
              <div className="flex items-center justify-between gap-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] text-[#006c49] font-bold">(Kho: {selectedVariant?.so_luong_ton || selectedVariant?.ton_kho || 150})</span>
                  </div>
                  <div className="flex items-center bg-white border border-slate-200 rounded-xl p-0.5 lg:p-1 shadow-sm">
                    ...
                    <button
                      onClick={() => {
                        const maxStock = selectedVariant?.so_luong_ton || selectedVariant?.ton_kho || 150;
                        if (quantity < maxStock) setQuantity(quantity + 1);
                        else alert(`Kho tại khu vực này chỉ còn tối đa ${maxStock} sản phẩm!`);
                      }}
                      disabled={!selectedVariant || quantity >= (selectedVariant?.so_luong_ton || selectedVariant?.ton_kho || 150)}
