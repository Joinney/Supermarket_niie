...
import { Trash2, Minus, Plus, ShoppingBag, ArrowLeft, CreditCard, ChevronDown, ChevronUp, Sparkles, AlertCircle } from 'lucide-react';
  ...
  if (loading) {
    return (
      <div className="min-h-[85vh] flex flex-col items-center justify-center bg-white">
        ...
        <p className="text-slate-400 font-extrabold tracking-widest mt-6 uppercase italic text-xs animate-pulse">Đang đồng bộ túi hàng Demi Mart...</p>
      </div>
    );
  }
      ...
      <div className="w-full max-w-[1250px] mx-auto px-4 sm:px-6 lg:px-8 pt-10 lg:pt-14">
        ...
        <div className="flex items-center gap-4 mb-10 border-b border-slate-100 pb-6 flex-wrap justify-between">
          <div className="flex items-center gap-4">
            <button onClick={() => navigate(-1)} className="p-3 bg-white border border-slate-200 hover:border-slate-300 hover:bg-slate-50 rounded-xl transition-all shadow-sm">
          ...
          <div className="lg:col-span-8 space-y-4">
            <div className="flex items-center justify-between bg-white border border-slate-100 px-5 py-4 rounded-xl shadow-sm">
              <label className="flex items-center gap-3 cursor-pointer select-none group">
                <input type="checkbox" checked={selectedItems.length === cart.length && cart.length > 0} onChange={toggleSelectAll} className="w-5 h-5 rounded accent-[#006c49] cursor-pointer border-slate-300" />
                  ...
                  return (
                    <div 
                      ...
                      className={`flex items-center gap-4 bg-white border p-4 rounded-2xl shadow-sm transition-all duration-300 ${
                        isSelected ? 'border-[#006c49]/40 bg-emerald-50/5 ring-2 ring-emerald-500/5' : 'border-slate-100'
                      }`}
                    >
                      ...
                      <div className="flex-1 min-w-0 flex items-center justify-between gap-4 flex-wrap sm:flex-nowrap">
                        ...
                        <div className="flex items-center gap-4 flex-shrink-0 w-full sm:w-auto justify-between sm:justify-end">
                          ...
                          <div className="flex items-center bg-slate-100 border border-slate-200/60 rounded-lg p-0.5 shadow-inner">
                            <button onClick={() => handleUpdateQuantity(singleItem, 'minus')} disabled={(singleItem.quantity || 1) <= 1} className="w-7 h-7 flex items-center justify-center text-slate-500 hover:text-slate-800 hover:bg-white rounded transition-all disabled:opacity-30">
                              <Minus size={10} strokeWidth={3} />
                            </button>
                            <span className="w-8 text-center font-black text-slate-800 text-xs">{singleItem.quantity || 1}</span>
                            <button onClick={() => handleUpdateQuantity(singleItem, 'plus')} className="w-7 h-7 flex items-center justify-center text-slate-500 hover:text-slate-800 hover:bg-white rounded transition-all">
                              <Plus size={10} strokeWidth={3} />
                            </button>
                          </div>

                          <button onClick={() => removeFromCart(singleItem.variantId)} className="text-slate-300 hover:text-red-500 p-2 hover:bg-red-50 rounded-xl transition-all">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-4 p-4 bg-gradient-to-r from-slate-50 via-slate-50/30 to-white justify-between">
                      <div className="flex items-center gap-4 flex-1 min-w-0">
                        ...
                        <div className="min-w-0 text-left">
                          ...
                          <p className="text-[11px] text-slate-400 mt-1 font-bold">
                            Tổng phân loại trong túi: <span className="text-[#006c49] font-black bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100 shadow-inner">{group.totalQuantity} mục</span>
                          </p>
                        </div>
                      </div>
                      ...
                      >
                        {isExpanded ? (
                          ...
                          <>Xem phân loại ({group.subVariants.length}) <ChevronDown size={13} strokeWidth={3} /></>
                        )}
                      </button>
                    </div>
                    ...
                    {isExpanded && (
                      <div className="divide-y divide-slate-100 bg-white border-t border-slate-50/50">
                        {group.subVariants.map((subItem) => {
                          const isSubSelected = selectedItems.includes(subItem.variantId);
                          const subProductDetailUrl = `/${country.toLowerCase()}/product/${category}/${group.productId}/${subItem.variantId}`;

                          return (
                            <div key={subItem.variantId} className={`flex items-center gap-4 p-4 transition-all duration-200 border-l-4 ${isSubSelected ? 'bg-emerald-50/5 border-l-[#006c49]' : 'border-l-transparent'}`}>
                              <input type="checkbox" checked={isSubSelected} onChange={() => toggleSelectItem(subItem.variantId)} className="w-4 h-4 rounded accent-[#006c49] cursor-pointer" />
                              ...
                              <div className="w-14 h-14 bg-slate-50 border border-slate-100 rounded-xl overflow-hidden p-1 flex-shrink-0 flex items-center justify-center shadow-sm">
                                <img src={subItem.image} alt={subItem.variantName} className="w-full h-full object-contain" />
                              </div>
                              ...
                              <div className="flex-1 min-w-0 flex items-center justify-between gap-4 flex-wrap sm:flex-nowrap">
                                ...
                                <div className="flex items-center gap-4 flex-shrink-0 w-full sm:w-auto justify-between sm:justify-end">
                                  ...
                                  <div className="flex items-center bg-slate-100 border border-slate-200/60 rounded-lg p-0.5 shadow-inner flex-shrink-0">
                                    <button onClick={() => handleUpdateQuantity(subItem, 'minus')} disabled={(subItem.quantity || 1) <= 1} className="w-7 h-7 flex items-center justify-center text-slate-500 hover:text-slate-800 hover:bg-white rounded transition-all disabled:opacity-30">
                                      <Minus size={10} strokeWidth={3} />
                                    </button>
                                    <span className="w-8 text-center font-black text-slate-800 text-xs">{subItem.quantity || 1}</span>
                                    <button onClick={() => handleUpdateQuantity(subItem, 'plus')} className="w-7 h-7 flex items-center justify-center text-slate-500 hover:text-slate-800 hover:bg-white rounded transition-all">
                                      <Plus size={10} strokeWidth={3} />
                                    </button>
                                  </div>

                                  <button onClick={() => removeFromCart(subItem.variantId)} className="text-slate-300 hover:text-red-500 p-2 hover:bg-red-50 rounded-xl transition-all flex-shrink-0">
                                    <Trash2 size={15} />
                                  </button>
                                </div>
                              </div>
            ...
            <div className="bg-white border border-slate-100 rounded-2xl p-6 lg:p-8 shadow-2xl shadow-slate-200/60 space-y-6 relative overflow-hidden">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-baseline pt-4 border-t border-dashed border-slate-200">
                  ...
                  <span className="text-xl lg:text-2xl font-black text-[#006c49] tracking-tighter" translate="no">
