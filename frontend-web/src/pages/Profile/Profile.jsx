...
import { 
  ...
  Star, Wallet, Ticket, Bell, Eye, History, Zap, Award, X, Plus, ChevronDown, Search, Loader2
} from "lucide-react";
...
export default function ProfilePage() {
  const [markerPos, setMarkerPos] = useState({ lat: 10.762622, lng: 106.660172 });
  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(async () => {
      ...
      let searchParams = {
        q: addressForm.detail_address.trim(), format: "json", limit: 6,
        addressdetails: 1, "accept-language": "vi", countrycodes: "vn"
      };
      if (addressForm.province_name) {
        ...
        if (addressForm.province_name.includes("Hồ Chí Minh") || addressForm.province_name.includes("HCM")) {
          searchParams.viewbox = "10.37,106.36,11.16,107.02"; searchParams.bounded = 1;
        } else if (addressForm.province_name.includes("Hà Nội")) {
          searchParams.viewbox = "20.61,105.28,21.36,106.03"; searchParams.bounded = 1;
        }
      }
      try {
        ...
        if (res.data && res.data.length > 0) {
          ...
          setAddressSuggestions(fallbackRes.data || []); setShowSuggestions(fallbackRes.data && fallbackRes.data.length > 0);
        }
      } catch (err) { console.error("Lỗi khi kết nối API bản đồ gợi ý:", err); }
  ...
  const selectWard = (code, name) => {
    ...
    setOpenDropdown(null); setSearchTerm('');
  };
  ...
  const handleOpenAddModal = () => {
    ...
    setAddressForm({
      receiver_name: profile?.full_name || "", receiver_phone: profile?.phone_number || "",
      province_name: "", province_id: "", district_name: "", district_id: "", ward_name: "", ward_code: "",
  ...
  const handleSelectSuggestion = (suggestion) => {
    ...
    setShowSuggestions(false); showToast("Đã đồng bộ tọa độ không gian bản đồ!");
  };
  ...
  const fetchAddressFromCoords = async (lat, lng) => {
    try {
      ...
      if (res.data && res.data.display_name) {
        ...
        setAddressForm(prev => ({ ...prev, detail_address: streetAddress || res.data.display_name }));
      }
    } catch (err) { console.log("Lỗi dịch ngược tọa độ bản đồ:", err); }
  };

  const handleOpenEditModal = async (addr) => {
    setEditingAddressId(addr.address_id);
    setAddressForm({ ...addr, ward_code: addr.ward_code || addr.ward_id || "", is_default: Boolean(addr.is_default) });
    if (addr.latitude && addr.longitude) setMarkerPos({ lat: parseFloat(addr.latitude), lng: parseFloat(addr.longitude) });
  ...
  const handleSaveAddress = async (e) => {
    e.preventDefault();
    if (!addressForm.province_id || !addressForm.district_id || !addressForm.ward_code) return alert("Vui lòng chọn đầy đủ danh mục địa chính từ bảng tìm kiếm!");
    try {
      const payload = { 
        ...addressForm, is_default: addressForm.is_default ? true : false,
        province_id: Number(addressForm.province_id), district_id: Number(addressForm.district_id),
    ...
    } catch (error) { showToast("Lỗi hệ thống khi xử lý địa chỉ", "error"); }
  };
  ...
  const handleSaveProfile = async () => {
    ...
    } catch (error) { showToast("Lỗi khi lưu dữ liệu!", "error"); }
  };
  ...
  const handleUploadCroppedAvatar = async () => {
    try {
      ...
      if (response.data.success) {
          ...
          showToast("Đã cập nhật ảnh đại diện tùy chỉnh rực rỡ!"); setImageSrc(null); 
      }
    } catch (error) { showToast("Lỗi xử lý cắt ảnh hệ thống", "error"); }
            ...
            <div className="p-6 bg-white space-y-5">
              <div className="space-y-2">
                <div className="flex justify-between text-[10px] font-black text-slate-400 uppercase tracking-wider">
                  <span>Thu nhỏ</span><span>Phóng to</span>
                </div>
          ...
          <div className="flex-1 w-full space-y-4">
            ...
            <div className="bg-white rounded-none md:rounded-[32px] shadow-sm border-none md:border border-slate-100 overflow-hidden flex flex-col min-h-screen md:min-h-[550px]">
              ...
              <div className="bg-[#fcfdfd] border-b border-slate-100 py-3 px-4 flex justify-around items-center overflow-x-auto no-scrollbar shrink-0">
                {orderSteps.map((step, i) => (
                  <div key={i} className="flex flex-col items-center gap-1 group cursor-pointer relative min-w-[70px]">
                    <div className="w-9 h-9 bg-white rounded-xl border border-slate-100 flex items-center justify-center text-slate-300 group-hover:text-[#006c49] transition-all">{step.icon}</div>
                    {step.count > 0 && <span className="absolute top-0 right-3 bg-red-500 text-white text-[7px] font-black w-3.5 h-3.5 rounded-full flex items-center justify-center border border-white">{step.count}</span>}
              ...
              <div className="px-5 md:px-8 lg:px-12 pb-10 pt-6 animate-fadeIn flex-1 text-left">
                ...
                {activeTab === "addresses" && (
                  <Tabdiachi 
                    addresses={addresses} handleOpenAddModal={handleOpenAddModal} handleOpenEditModal={handleOpenEditModal} handleDeleteAddress={handleDeleteAddress} handleSetDefault={handleSetDefault}
                    isAddressModalOpen={isAddressModalOpen} setIsAddressModalOpen={setIsAddressModalOpen} editingAddressId={editingAddressId} handleSaveAddress={handleSaveAddress} addressForm={addressForm} setAddressForm={setAddressForm}
                    openDropdown={openDropdown} setOpenDropdown={setOpenDropdown} searchTerm={searchTerm} setSearchTerm={setSearchTerm} filteredProvinces={filteredProvinces} selectProvince={selectProvince}
                    filteredDistricts={filteredDistricts} selectDistrict={selectDistrict} filteredWards={filteredWards} selectWard={selectWard} showSuggestions={showSuggestions} setShowSuggestions={setShowSuggestions}
                    addressSuggestions={addressSuggestions} isLoadingSuggestions={isLoadingSuggestions} handleSelectSuggestion={handleSelectSuggestion} markerPos={markerPos} setMarkerPos={setMarkerPos} fetchAddressFromCoords={fetchAddressFromCoords}
                    provinceRef={provinceRef} districtRef={districtRef} wardRef={wardRef} suggestionRef={suggestionRef}
                  />
                )}

                {activeTab === "security" && (
                  <Tabbaomat 
                    securityStep={securityStep} setSecurityStep={setSecurityStep} currentPassword={currentPassword} setCurrentPassword={setCurrentPassword} handleVerifyCurrentPassword={handleVerifyCurrentPassword}
                    profile={profile} handleSendOTP={handleSendOTP} otpCode={otpCode} setOtpCode={setOtpCode} handleVerifyOTP={handleVerifyOTP} newPassword={newPassword} setNewPassword={setNewPassword}
                    confirmNewPassword={confirmNewPassword} setConfirmNewPassword={setConfirmNewPassword} handleResetPassword={handleResetPassword}
                  />
                )}
