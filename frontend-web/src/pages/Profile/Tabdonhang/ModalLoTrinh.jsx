...
export default function ModalLoTrinh({ isOpen, onClose, order }) {
  ...
  const userLat = parseFloat(
    order?.to_lat || order?.latitude || order?.user_lat || 10.762622,
  );
  const userLng = parseFloat(
    order?.to_lng || order?.longitude || order?.user_lng || 106.660172,
  );
    ...
    const renderRouteMap = async () => {
      ...
      try {
        ...
        const authBaseUrl = isLocalHost
          ...
          : "https://authservice-sz4p.onrender.com/api";
        ...
        // [1] LUỒNG ĐỒNG BỘ AVATAR - DÙNG authApi (CHUẨN v1 & TỰ ĐỘNG TOKEN)
        let targetAvatar =
        ...
        if (!targetAvatar && order.user_id) {
          try {
            // ✅ Dùng authApi.get thay vì axios.get + header thủ công
            const userProfileRes = await authApi.get(
              `/auth/internal/users/${order.user_id}`,
            );
            if (
            ...
            console.warn(
              "⚠️ Không thể kéo profile avatar từ Auth Service:",
              authFetchErr.message,
            );
          }
        }
        ...
        // [2] Gọi API đồng bộ Address - DÙNG authApi
        try {
          // ✅ Dùng authApi với path tương đối, tự động nhận token
          ...
          if (Array.isArray(addrDataList) && addrDataList.length > 0) {
            const matchedAddr =
              addrDataList.find(
                (addr) =>
                  Number(addr.district_id) === Number(order.to_district_id),
              ) ||
            ...
            if (matchedAddr && isMounted) {
              setAddressData({
                receiver_name:
                  matchedAddr.receiver_name || "Khách hàng DemiMart",
                receiver_phone:
                  matchedAddr.receiver_phone || "Chưa cập nhật SĐT",
        ...
        // [3] Gọi API lấy tính toán khoảng cách gốc - DÙNG orderApi
        try {
          const res = await orderApi.post("/orders/shipping/calc", {
            userLat,
            userLng,
          });
        ...
        try {
          const postOfficeRes = await orderApi.post("/orders/test-kml", {
            district_name: "",
            province_name: "",
          });
        ...
        if (rawPostOffices.length > 0) {
          let clientZoneOffices = rawPostOffices.filter((o) => {
            ...
            return (
              Math.abs(lat - userLat) < 0.6 && Math.abs(lng - userLng) < 0.6
            );
          });
          ...
          if (clientZoneOffices.length === 0) {
            clientZoneOffices = rawPostOffices.filter((o) => {
              ...
              const lng = parseFloat(o.location?.lng || o.longitude || 0);
              return (
                Math.abs(lat - userLat) < 1.5 && Math.abs(lng - userLng) < 1.5
              );
            });
          }
          ...
          clientZoneOffices.forEach((office) => {
            const oLat = parseFloat(
              office.location?.lat || office.latitude || 0,
            );
            const oLng = parseFloat(
              office.location?.lng || office.longitude || 0,
            );
          ...
          lastMileLat = parseFloat(
            optimalLastMileOffice.location?.lat ||
              optimalLastMileOffice.latitude ||
              userLat,
          );
          lastMileLng = parseFloat(
            optimalLastMileOffice.location?.lng ||
              optimalLastMileOffice.longitude ||
              userLng,
          );
          lastMileName =
            optimalLastMileOffice.name || "Bưu cục chặng cuối DemiMart Express";
        }
        ...
        if (!leafletMapInstance.current && mapRef.current) {
          ...
          L.tileLayer(
            "https://{s}.google.com/vt/lyrs=m&x={x}&y={y}&z={z}&hl=vi&gl=VN",
            {
              maxZoom: 20,
          ...
          routingLayer.current = L.featureGroup().addTo(
            leafletMapInstance.current,
          );
        }
        ...
        if (routeData.code === "Ok" && isMounted && routingLayer.current) {
          ...
          L.marker([storeLat, storeLng], { icon: customShopIcon })
            .bindPopup(`<b>${storeName}</b><br/>Kho xuất phát tổng`)
            .addTo(routingLayer.current);
          ...
          if (lastMileLat && lastMileLng) {
            L.marker([lastMileLat, lastMileLng], {
              icon: customDistrictTruckIcon,
            })
              .bindPopup(
                `<b>🏁 ${lastMileName}</b><br/>Bưu cục chặng cuối phụ trách phát hàng`,
              )
              .addTo(routingLayer.current);
          }
          ...
          if (coordinates.length > 15) {
            ...
            distributionRatios.forEach((ratio, idx) => {
              ...
              if (coordinates[targetIndex]) {
                ...
                if (lastMileLat && Math.abs(nodeCoord[0] - lastMileLat) < 0.05)
                  return;
          ...
          if (leafletMapInstance.current && routingLayer.current) {
            leafletMapInstance.current.fitBounds(
              routingLayer.current.getBounds(),
              { padding: [50, 50] },
            );
          }
        }
      ...
      <div className="bg-white w-screen h-screen flex flex-col overflow-hidden max-h-screen">
        ...
        <div className="p-4 border-b flex justify-between items-center bg-[#006c49] text-white shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-sm">
              Hệ thống Quản lý và Tối ưu Lộ trình Logistics
            </h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-3 bg-emerald-50/50 flex justify-between items-center border-b shrink-0 text-xs">
          <div className="flex items-center gap-2 max-w-[45%]">
            <div className="bg-emerald-100 p-2 rounded-full text-[#006c49] shrink-0">
              <MapPin size={16} />
            </div>
            <div className="truncate">
              <span className="text-[10px] font-bold text-slate-400 uppercase block">
                Bưu cục chặng đích phát hàng
              </span>
        ...
        <div className="flex flex-1 flex-row w-full h-full overflow-hidden relative">
            ...
            {routeInfo.loading && (
              <div className="absolute inset-0 z-[1000] bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center gap-2 text-[#006c49]">
                ...
                <span className="text-xs font-black uppercase tracking-widest">
                  Đang tính toán ma trận thực địa đường đi...
                </span>
              </div>
            )}
            <div
          ...
          <div
            className={`h-full bg-slate-50 border-l border-slate-200 flex flex-col overflow-y-auto p-5 gap-4 shadow-inner transition-all duration-300 ${
              isSidebarOpen
                ? "w-[32%] opacity-100 visible"
                : "w-0 p-0 opacity-0 invisible border-l-0"
            }`}
          >
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-3">
              <h5 className="font-bold text-xs text-slate-400 uppercase tracking-wider flex items-center gap-1.5 border-b pb-2">
                <User size={14} className="text-slate-500" /> Thông tin người
                đặt nhận hàng
              </h5>
              <div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-3">
              <h5 className="font-bold text-xs text-slate-400 uppercase tracking-wider flex items-center gap-1.5 border-b pb-2">
                <Package size={14} className="text-slate-500" /> Danh sách sản
                phẩm mua ({finalGroupedItems.length})
              </h5>
              <div className="flex flex-col gap-3 max-h-[250px] overflow-y-auto pr-1">
                {finalGroupedItems.length > 0 ? (
                  finalGroupedItems.map((group, index) => (
                    <div
                      key={index}
                      className="flex gap-3 p-2 bg-slate-50 border border-slate-100 rounded-xl text-xs items-center"
                    >
                      <img
                        src={
                          group.image_url ||
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[10px] text-slate-400 font-medium mt-0.5 leading-relaxed truncate">
                          ...
                          {group.variants.map((v, i) => (
                            <span key={i}>
                              {v.name} •{" "}
                              <b className="text-slate-600">x{v.qty}</b>
                  ...
                  <div className="text-slate-400 font-medium text-center py-4 text-[11px]">
                    Không tìm thấy chi tiết kiện hàng.
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-1 border-t pt-3">
                <div className="bg-slate-50/60 p-2 rounded-xl border border-slate-100">
                  ...
                  <span className="font-bold text-slate-700 text-xs">
                    {order?.khoi_luong ? `${order.khoi_luong} kg` : "0.5 kg"}
                  </span>
                </div>
                <div className="bg-slate-50/60 p-2 rounded-xl border border-slate-100">
                  <span className="text-slate-400 text-[10px] block">
                  ...
                  <span className="font-bold text-slate-700 text-[11px] truncate block">
                    {order?.hinh_thuc_thanh_toan || "Thẻ tín dụng / COD"}
                  </span>
                </div>
              </div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-2 text-xs">
              <div className="flex justify-between text-slate-500">
                <span className="flex items-center gap-1">
                  <Calendar size={13} /> Ngày đặt vận đơn:
                </span>
                <span className="font-medium text-slate-700">
                  {new Date(order?.ngay_tao || Date.now()).toLocaleDateString(
                    "vi-VN",
                  )}
                </span>
              </div>
              <div className="flex justify-between text-slate-500">
                <span className="flex items-center gap-1">
                  <Truck size={13} /> Đơn vị vận chuyển:
                </span>
                <span className="font-semibold text-[#006c49]">
                  {order?.don_vi_van_chuyen || "DemiMart Logistics"}
                </span>
              </div>
            </div>
          </div>
