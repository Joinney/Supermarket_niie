...
// Component con cập nhật lại khung nhìn bản đồ khi tọa độ thay đổi
...
// Component con xử lý Marker có thể kéo thả trên bản đồ
...
export default function AddressModal({
  ...
  // Nếu modal không được mở, không render bất cứ dữ liệu nào
  if (!isOpen) return null;

  // Sử dụng createPortal để đưa Modal lên trên cùng cấu trúc DOM (Body)
  return createPortal(
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-[2px] animate-fadeIn">
      <div className="bg-white w-full max-w-lg rounded-[32px] shadow-2xl overflow-hidden border border-slate-100 relative z-10">
        ...
        <div className="p-6 border-b flex justify-between items-center bg-white">
          ...
          <button onClick={onClose} className="text-slate-300 hover:text-red-500 transition-all">
            <X size={20}/>
          </button>
        </div>
        ...
        <form onSubmit={handleSaveAddress} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto pr-2 no-scrollbar">
          ...
          {/* Ô nhập địa chỉ chi tiết & Gợi ý Mapbox/Nominatim */}
