  ...
  useEffect(() => {
    const fetchSubCategories = async () => {
      try {
        ...
        const currentCategory = allCategories.find(c => c.slug === targetParentSlug);
    ...
    <div className="max-w-[1400px] mx-auto p-4 md:p-6 font-sans bg-white min-h-screen">
      ...
      {subCategories.length > 0 && (
        <div className="relative w-full mb-6 group/subnav">
          ...
          <div ref={sliderRef} className="flex gap-2.5 overflow-x-auto pb-1 snap-x scroll-smooth" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            ...
            {subCategories.map((sub) => (
              ...
              >
                <span className="text-xs font-bold text-slate-800 line-clamp-2 pr-1">{sub.name}</span>
              </button>
            ))}
          </div>
      ...
      <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-100">
        <div className="text-sm font-semibold text-slate-600">{products.length} kết quả</div>
        <div className="flex items-center gap-2">
          <span className="text-xs text-slate-400">Sắp xếp:</span>
          <select value={selectedSort} onChange={(e) => setSelectedSort(e.target.value)} className="text-xs font-semibold text-slate-700 bg-slate-50 border border-slate-200 rounded-md px-2 py-1.5 focus:outline-none focus:border-[#006c49]">
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
        ...
        {/* KHỐI NỘI DUNG SẢN PHẨM PHÍA BÊN TRÁI */}
        <div className="flex-1 w-full order-2 lg:order-1 border border-slate-100 rounded-2xl p-4 bg-white shadow-sm flex flex-col justify-between min-h-[550px]">
          <div>
            {loading ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {products.map(p => <ProductCard key={p.ma_san_pham} p={p} />)}
              </div>
            )}
          </div>
          ...
          {!loading && products.length > 0 && totalPages > 1 && (
            <div className="flex items-center justify-center gap-1.5 mt-10 pt-5 border-t border-slate-100 w-full select-none">
              <button onClick={() => handlePageChange(1)} disabled={currentPage === 1} className="p-2 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 disabled:opacity-40 disabled:hover:bg-white transition-colors">
                <ChevronsLeft size={16} />
              </button>
              <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="p-2 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 disabled:opacity-40 flex items-center gap-1 text-xs font-semibold">
                <ChevronLeft size={16} /> Trước
              </button>
              {[...Array(totalPages)].map((_, index) => {
                const pageNum = index + 1;
                return (
                  <button key={pageNum} onClick={() => handlePageChange(pageNum)} className={`w-9 h-9 rounded-lg border text-xs font-bold transition-all ${currentPage === pageNum ? 'bg-[#006c49] border-[#006c49] text-white shadow-sm shadow-emerald-700/20' : 'border-slate-200 text-slate-700 hover:bg-slate-50 hover:border-slate-300'}`}>
                    {pageNum}
                  </button>
                );
              })}
              <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="p-2 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 disabled:opacity-40 flex items-center gap-1 text-xs font-semibold">
                Sau <ChevronRight size={16} />
              </button>
              <button onClick={() => handlePageChange(totalPages)} disabled={currentPage === totalPages} className="p-2 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 disabled:opacity-40 disabled:hover:bg-white transition-colors">
        ...
        <div className="w-full lg:w-64 border border-slate-100 rounded-xl p-4 bg-white shadow-sm order-1 lg:order-2 sticky top-4">
          <div className="flex items-center justify-between mb-4 pb-2 border-b border-slate-100">
            ...
            <button onClick={() => { setSelectedPrice("tat-ca"); setSelectedOrigin("tat-ca"); setShippingMethod({fresh:false, global:false})}} className="text-xs text-slate-400 hover:text-red-500 transition-colors">Đặt lại</button>
          </div>
          <div className="mb-5">
            <h4 className="text-xs font-bold text-slate-700 mb-2">Phương thức giao hàng</h4>
            <div className="space-y-2">
              <label className="flex items-center gap-2 text-xs text-slate-600 cursor-pointer">
                <input type="checkbox" checked={shippingMethod.fresh} onChange={(e) => setShippingMethod({...shippingMethod, fresh: e.target.checked})} className="rounded text-[#006c49] focus:ring-[#006c49] w-3.5 h-3.5" /> Giao hàng hoả tốc
              </label>
              <label className="flex items-center gap-2 text-xs text-slate-600 cursor-pointer">
                <input type="checkbox" checked={shippingMethod.global} onChange={(e) => setShippingMethod({...shippingMethod, global: e.target.checked})} className="rounded text-[#006c49] focus:ring-[#006c49] w-3.5 h-3.5" /> <span className="bg-orange-500 text-white text-[10px] px-1 rounded font-black scale-90">GLOBAL</span>
              </label>
            </div>
          </div>
          <div className="mb-5">
            ...
            <div className="space-y-2">
              {[{ label: 'Tất cả khoảng giá', value: 'tat-ca' }, { label: 'Dưới 50.000đ', value: '0-50000' }, { label: '50.000đ - 100.000đ', value: '50000-100000' }, { label: '100.000đ - 200.000đ', value: '100000-200000' }, { label: 'Trên 200.000đ', value: '200000-up' }].map((item) => (
                <label key={item.value} className="flex items-center gap-2 text-xs text-slate-600 cursor-pointer">
                  <input type="radio" name="price-filter" value={item.value} checked={selectedPrice === item.value} onChange={() => setSelectedPrice(item.value)} className="text-[#006c49] focus:ring-[#006c49] w-3.5 h-3.5" /> {item.label}
                </label>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="space-y-2">
              {[{ label: 'Tất cả', value: 'tat-ca' }, { label: 'Việt Nam', value: 'vn' }, { label: 'Nhập Khẩu', value: 'nhap-khau' }, { label: 'Nhật Bản', value: 'jp' }, { label: 'Hàn Quốc', value: 'kr' }].map((item) => (
                <label key={item.value} className="flex items-center gap-2 text-xs text-slate-600 cursor-pointer">
                  <input type="radio" name="origin-filter" value={item.value} checked={selectedOrigin === item.value} onChange={() => setSelectedOrigin(item.value)} className="text-[#006c49] focus:ring-[#006c49] w-3.5 h-3.5" /> {item.label}
                </label>
              ))}
            </div>
          </div>
