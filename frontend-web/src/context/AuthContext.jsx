...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (username, password) => {
    ...
    try {
      ...
      return {
        success: false,
        message: "Server không phản hồi mã Token truy cập!",
      };
    } catch (error) {
      console.error(
        "❌ [AUTH] Lỗi Đăng nhập thất bại:",
        error.response?.data?.message || error.message,
      );
      return {
        success: false,
  ...
  // 🌟 THÊM: HÀM TIỆN ÍCH LẤY HẠNG VIP NHANH
  const getMembershipTier = useCallback(() => {
    if (!user || String(user.role).toLowerCase() !== "buyer") return null;
    return user.membership_tier || "BẠC";
  }, [user]);
