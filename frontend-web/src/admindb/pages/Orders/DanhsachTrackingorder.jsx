...
export default function DanhsachTrackingorder() {
  ...
  const fetchTrackingOrders = async () => {
    ...
    try {
      ...
      const statsResponse = await orderApi.get("/admin/statistics", {
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <div className="text-lg font-black text-slate-700 mt-2 truncate">
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-visible">
        ...
        <div className="w-full overflow-x-auto min-h-[400px] relative">
          ...
          {error && (
            <div className="text-center p-8 text-sm font-black text-rose-500">
              {error}
            </div>
          )}
          ...
          {!loading && orders.length === 0 && (
            <div className="text-center py-12 text-xs font-bold text-slate-400">
              Hệ thống chưa ghi nhận dữ liệu kiện hàng nào khớp bộ lọc.
            </div>
          )}
              ...
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100 text-[10px] font-black uppercase tracking-wider text-slate-400 select-none">
                  ...
                  <th className="py-4 px-4 min-w-[240px]">
                    Hình thức nhận hàng (Thanh toán)
                  </th>
                  <th className="py-4 px-4 min-w-[220px]">
                    Trạm hiện tại / Kho trung chuyển
                  </th>
              ...
              <tbody className="divide-y divide-slate-50 text-xs font-semibold text-slate-700">
                {orders.map((order, idx) => {
                  ...
                  return (
                    <tr
                      key={order.id || idx}
                      className={`transition-colors group ${isSelected ? "bg-emerald-50/20" : "hover:bg-slate-50/50"}`}
                    >
                      ...
                      <td
                        className="py-4 px-4 font-black text-emerald-700 font-mono whitespace-nowrap align-middle cursor-pointer hover:underline"
                      ...
                      <td className="py-4 px-4 align-middle">
                        <div className="font-bold text-slate-900">
                          {order.phuong_thuc_thanh_toan || "Ví điện tử / COD"}
                        </div>
                        <div className="text-[10px] text-slate-400 font-medium">
                          Đối soát: {order.trang_thai_thanh_toan}
                        </div>
                      </td>

                      <td className="py-4 px-4 align-middle font-medium text-slate-600">
                        <div
                          className="truncate max-w-[210px] text-[11px] font-bold text-slate-700"
                          title={order.tram_hien_tai}
                        >
                          {order.tram_hien_tai}
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-4 whitespace-nowrap text-center align-middle">
                        <span
                          className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wide ${getTrackingBadgeClass(order.trang_thai_don_hang)}`}
                        >
