...
export default function GeneralSettings() {
  ...
  // Try to load some real settings (VIP threshold) from Auth Service if available
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await authApi.get("/settings/vip");
        if (mounted && res?.data?.value) {
          // auth service returns value as JSON string or number
          const raw = res.data.value;
          const threshold = Number(raw);
          if (!Number.isNaN(threshold)) {
            setSettings((prev) => ({ ...prev, vipThreshold: threshold }));
            setDataSource("real");
          }
        }
      } catch (err) {
        // silently ignore — keep mock data
      }
    })();
    return () => {
      mounted = false;
  ...
  const handleSave = async () => {
    // Basic validation
    const newErrors = {};
    if (!/^[\w.-]+@[\w.-]+\.[A-Za-z]{2,}$/.test(settings.email))
      newErrors.email = "Email không hợp lệ";
    if (settings.taxRate < 0 || settings.taxRate > 100)
      newErrors.taxRate = "Thuế suất phải từ 0-100";
    ...
    try {
      if (settings.vipThreshold !== undefined) {
        await authApi.put("/settings/vip", {
          value: Number(settings.vipThreshold),
        });
      }
      // TODO: call other services if implemented (system_settings endpoint)
      setSaveMessage("Cập nhật cài đặt thành công");
      setDataSource("real");
      setTimeout(() => setSaveMessage(""), 3000);
    } catch (err) {
      console.error("Lỗi lưu settings:", err);
      setSaveError("Lưu cài đặt thất bại — kiểm tra console để biết chi tiết");
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 font-sans text-gray-800">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={handleChange}
                  className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none bg-white"
                >
          ...
          <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <div className="md:col-span-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-gray-400">
                    Nguồn dữ liệu:{" "}
                    <strong className="text-emerald-600">{dataSource}</strong>
                  </span>
                </div>
              </div>
              ...
              <div className="md:col-span-2 flex items-center justify-between rounded-xl bg-gray-50 p-4">
                ...
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${settings.isTaxIncluded ? "translate-x-6" : "translate-x-1"}`}
                  />
                </button>
              </div>
            </div>
          ...
          <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <div className="md:col-span-1 flex items-center justify-between rounded-xl bg-gray-50 p-4">
                ...
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${settings.is2FA ? "translate-x-6" : "translate-x-1"}`}
                  />
                </button>
              </div>
              <div className="md:col-span-2">
                ...
                <select
                  ...
                  className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none bg-white"
                >
                  <option value="Sau 1 giờ không hoạt động">
                    Sau 1 giờ không hoạt động
                  </option>
                  <option value="Sau 2 giờ không hoạt động">
                    Sau 2 giờ không hoạt động
                  </option>
              ...
              <div className="md:col-span-3">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none placeholder:text-gray-300"
                />
              </div>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={handleChange}
                  className="flex-1 rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none text-gray-600"
                />
                <button
                  type="button"
                  onClick={() =>
                    alert(`Đang kiểm tra kết nối tới: ${settings.webhookUrl}`)
                  }
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none bg-white"
                >
                  <option value="VND - Việt Nam Đồng (đ)">
                    VND - Việt Nam Đồng (đ)
                  </option>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none bg-white"
                >
                  <option value="DD/MM/YYYY (31/12/2023)">
          ...
          <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    value={settings.hoursMonFriStart}
                    onChange={handleChange}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <span className="rounded bg-rose-50 px-2 py-1 text-xs font-semibold text-rose-500 uppercase">
                    Đóng cửa
                  </span>
                  <button
                    type="button"
              ...
              <button
                type="button"
                onClick={() =>
                  alert("Chức năng thêm ngày nghỉ lễ đang phát triển.")
                }
                className="mt-4 w-full rounded-xl border border-dashed border-emerald-500 py-2.5 text-sm font-medium text-emerald-600 hover:bg-emerald-50/50 transition"
              >
                + Thêm ngày nghỉ Lễ
              </button>
            </div>
          ...
          <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <div
                    className="h-8 w-8 rounded-lg bg-[#007A5A] shadow-inner cursor-pointer border border-black/5"
                    title="#007A5A"
                  />
                  <div
                    className="h-8 w-8 rounded-lg bg-white shadow-inner cursor-pointer border border-gray-200"
                    title="#FFFFFF"
                  />
                  <button
                    type="button"
                    className="flex h-8 w-8 items-center justify-center rounded-full border border-dashed border-gray-300 text-gray-400 hover:border-gray-400 hover:text-gray-600 transition"
                  >
                    +
                  </button>
                </div>
      ...
      {/* FIXED BOTTOM BAR */}
      <div className="fixed bottom-4 left-1/2 z-40 w-[92%] max-w-5xl -translate-x-1/2 rounded-xl bg-white p-4 border border-gray-100 shadow-lg">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              onClick={handleSave}
              className={clsx(
                "rounded-lg px-5 py-2 text-sm font-semibold text-white transition shadow-sm",
                saving
                  ? "bg-emerald-300 cursor-not-allowed"
                  : "bg-[#007A5A] hover:bg-[#006349]",
              )}
              disabled={saving}
            >
      ...
      {/* TOAST */}
      {saveMessage && (
        <div className="fixed top-6 right-6 z-50 rounded-md bg-emerald-50 border border-emerald-200 px-4 py-2 text-emerald-700 shadow">
          {saveMessage}
        </div>
      )}
      {saveError && (
        <div className="fixed top-6 right-6 z-50 rounded-md bg-rose-50 border border-rose-200 px-4 py-2 text-rose-700 shadow">
          {saveError}
        </div>
      )}
    </div>
