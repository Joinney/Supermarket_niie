...
// 🌟 THÊM: Import bộ icon vẽ nét đồng bộ từ lucide-react
import {
  Users,
  Shield,
  Box,
...
export default function Danhsachnoibo() {
  ...
  const renderModuleIcon = (type) => {
    ...
    switch (type) {
      ...
      case "products": return <ShoppingBag className={iconClass} />;
      case "orders": return <ClipboardList className={iconClass} />;
      case "inventory": return <Warehouse className={iconClass} />;
      case "customers": return <Users className={iconClass} />;
      case "settings": return <Lock className="w-4 h-4 text-red-400 stroke-[2.2]" />;
    ...
    <div className="w-full bg-[#fafafa] font-sans antialiased text-slate-800 text-left relative p-4 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2.5 self-start sm:self-center">
          ...
          <button onClick={() => setIsCollapsedModal(true)} className="flex items-center gap-1.5 bg-[#006c49] text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:bg-[#00563a] transition-all cursor-pointer">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-white p-5 rounded-2xl border border-gray-100 flex items-center justify-between shadow-sm">
          ...
          <div className="w-10 h-10 bg-emerald-50 text-[#006c49] rounded-xl flex items-center justify-center shadow-sm"><Users className="w-5 h-5 stroke-[2.2]" /></div>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-gray-100 flex items-center justify-between shadow-sm">
          ...
          <div className="w-10 h-10 bg-red-50 text-red-500 rounded-xl flex items-center justify-center shadow-sm"><Shield className="w-5 h-5 stroke-[2.2]" /></div>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-gray-100 flex items-center justify-between shadow-sm">
          ...
          <div className="w-10 h-10 bg-blue-50 text-blue-500 rounded-xl flex items-center justify-center shadow-sm"><Box className="w-5 h-5 stroke-[2.2]" /></div>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-gray-100 flex items-center justify-between shadow-sm">
          ...
          <div className="w-10 h-10 bg-emerald-50 text-emerald-500 rounded-xl flex items-center justify-center shadow-sm"><UserCheck className="w-5 h-5 stroke-[2.2]" /></div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-[2px] z-[9999] flex items-center justify-center p-4 select-none animate-fadeIn">
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-xl border border-gray-100 overflow-hidden transform transition-all duration-300">
            <div className="p-6 pb-4 flex items-center justify-between border-b border-gray-50">
              ...
              <button onClick={() => { setIsCollapsedModal(false); setAvatarPreview(""); setAvatarFile(null); }} className="w-7 h-7 flex items-center justify-center rounded-full bg-gray-50 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition cursor-pointer"><X className="w-3.5 h-3.5 stroke-[2.5]" /></button>
            </div>
            ...
            <form onSubmit={handleCreateAccount} className="p-6 space-y-4 text-sm font-semibold text-slate-700">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
                ...
                <div className="flex flex-col gap-1.5 relative">
                  ...
                  <div className="relative">
                    ...
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-slate-600 flex items-center justify-center cursor-pointer p-1 rounded-md">
              ...
              <div className="flex items-center justify-end gap-2.5 pt-4 border-t border-gray-50">
                <button type="button" onClick={() => { setIsCollapsedModal(false); setAvatarPreview(""); setAvatarFile(null); }} className="px-5 py-2.5 border border-gray-200 hover:bg-gray-50 rounded-xl text-xs font-bold text-gray-500 transition cursor-pointer">Hủy bỏ</button>
                <button type="submit" className="px-5 py-2.5 bg-[#006c49] text-white rounded-xl text-xs font-bold shadow-sm hover:bg-[#00563a] transition flex items-center gap-1.5 cursor-pointer"><span>+</span> Cấp tài khoản</button>
              </div>
            </form>
          </div>
        </div>,
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[9999] flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white rounded-3xl w-full max-w-3xl shadow-2xl border border-slate-100 overflow-hidden transform transition-all max-h-[92vh] flex flex-col">
            ...
            <div className="p-6 pb-4 flex items-center justify-between border-b border-gray-100 shrink-0">
              ...
              <button onClick={() => setIsPermissionModalOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-full bg-slate-50 text-gray-400 hover:bg-slate-100 hover:text-gray-600 transition cursor-pointer"><X className="w-4 h-4 stroke-[2.5]" /></button>
            </div>
              ...
              <div className="bg-slate-50/50 p-4 rounded-2xl border border-gray-100">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="flex flex-wrap items-center gap-2 lg:gap-2.5 self-start sm:self-center">
                    <button type="button" onClick={() => handleApplyPreset(selectedUser.role)} className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-gray-200 hover:bg-gray-50 text-slate-700 rounded-xl text-xs font-bold transition shadow-sm cursor-pointer">
                      Chọn mặc định
                    </button>
                    <button type="button" onClick={handleSelectAllPermissions} className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-[#006c49] border border-emerald-200 rounded-xl text-xs font-bold transition shadow-sm cursor-pointer">
                      Chọn tất cả
                    </button>
                    <button type="button" onClick={handleClearAllPermissions} className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 border border-red-100 rounded-xl text-xs font-bold transition shadow-sm cursor-pointer">
                      Xóa tất cả
                    </button>
                  </div>
                </div>
            ...
            <div className="p-5 flex items-center justify-end gap-3 bg-slate-50/50 border-t border-gray-100 shrink-0">
              <button type="button" onClick={() => setIsPermissionModalOpen(false)} className="px-6 py-2.5 border border-gray-200 bg-white rounded-xl text-xs font-bold text-gray-500 transition shadow-sm cursor-pointer">Hủy bỏ</button>
              <button type="button" onClick={handleSavePermissions} className="px-6 py-2.5 bg-[#006c49] text-white rounded-xl text-xs font-bold shadow-sm hover:bg-[#00563a] transition flex items-center gap-2 cursor-pointer">
