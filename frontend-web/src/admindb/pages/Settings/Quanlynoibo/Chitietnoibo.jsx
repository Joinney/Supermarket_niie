...
export default function Chitietnoibo() {
    ...
    try {
      const targetId =
        ...
        (location.state?.user?.user_id
          ? String(location.state.user.user_id)
          : null) ||
        location.state?.user?.id ||
        "1";

      // ✅ Dùng authApi (tự động đính kèm Token và trỏ tới /api/v1/auth)
      const [userRes, groupRes] = await Promise.all([
        authApi.get(`/auth/internal/users/${targetId}`),
        authApi.get(`/auth/internal/users/role-group/${targetId}`),
      ]);
      ...
      setError("Không thể tải dữ liệu nhân sự (kiểm tra lại cổng 5001).");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      await authApi.put(`/auth/internal/users/${targetId}`, payload);

      setIsEditing(false);
      alert("Đã đồng bộ cập nhật xuống PostgreSQL thành công!");
      fetchUserData();
    } catch (err) {
  ...
  const renderModuleIcon = (type) => {
    ...
    switch (type) {
        ...
        return (
          ...
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2-2V6.75a2.25 2.25 0 0 0-2-2h-2.25m-3 0H7.5a2.25 2.25 0 0 0-2 2v12a2.25 2.25 0 0 0 2 2h2.25m3.75-16.5a1.5 1.5 0 0 0-3 0v1.5a1.5 1.5 0 0 0 3 0v-1.5Z"
            />
          </svg>
        );
      case "inventory":
      ...
      case "settings":
        return (
          <svg
            className="w-[18px] h-[18px] text-red-500 shrink-0"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth="1.6"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M6 3v6m0 4v8M4 9h4M12 3v2m0 4v12M10 5h4M18 3v10m0 4v4M16 13h4"
            />
          </svg>
        );
      default:
  ...
  return (
    <div className="w-full bg-[#fafafa] font-sans antialiased text-slate-800 text-left p-1 relative">
      <div className="absolute top-2 right-4 text-[10px] font-bold text-gray-400">
        {submitting
          ? "⏳ Đồng bộ Database..."
          : `🆔 Mã số: ${userInfo.userId || "Đang tải"}`}
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-5">
            <div className="flex items-center gap-2 border-b border-gray-100 pb-3">
              <svg
                className="w-[18px] h-[18px] text-[#006c49]"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth="1.6"
              ...
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide">
                Thông tin hồ sơ nhân viên (`public.users`)
              </h3>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              {userInfo.avatarUrl ? (
                <img
                  ...
                  alt="Avatar"
                  className="w-16 h-16 rounded-full object-cover border border-emerald-100 shadow-sm bg-slate-50"
                />
              ) : (
              ...
              <div>
                <h4 className="text-base font-bold text-slate-800 uppercase">
                  {userInfo.fullName || "Đang kết nối..."}
                </h4>
                <p className="text-xs text-gray-400 mt-0.5">
                  Vai trò hệ thống:{" "}
                  <strong className="text-[#006c49] uppercase font-mono">
                    {userInfo.role}
                  </strong>
                </p>
              </div>
            ...
            <div className="flex items-center gap-2 border-b border-gray-100 pb-3">
              ...
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M9 12.75L11.25 15 15 9.75M21 12c0 1.268-.63 2.39-1.593 3.068a3.745 3.745 0 01-1.043 3.296 3.745 3.745 0 01-3.296 1.043A3.745 3.745 0 0112 21c-1.268 0-2.39-.63-3.068-1.593a3.746 3.745 0 01-3.296-1.043 3.745 3.745 0 01-1.043-3.296A3.745 3.745 0 0121 12Z"
                />
              </svg>
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide">
              ...
              <table className="w-full border-collapse text-xs">
                <thead>
                  <tr className="bg-slate-50 border-b border-gray-100 text-[10px] font-extrabold uppercase tracking-wider text-gray-400 select-none">
                    <th className="py-3 px-5 w-1/2 text-left">
                      Mô-đun chức năng
                    </th>
                ...
                <tbody className="divide-y divide-gray-50 font-bold text-slate-700">
                  {matrixPermissions.map((item) => (
                    ...
                    >
                      ...
                      {["view", "add", "edit", "delete", "approve"].map(
                        (field) => (
                          <td key={field} className="py-3 px-2 text-center">
                            <input
                              ...
                              onChange={() =>
                                handleCheckboxChange(item.id, field)
                              }
          ...
          <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-4">
            ...
            <div className="space-y-2 max-h-[260px] overflow-y-auto pr-1 custom-scrollbar">
              {filteredAssignedUsers.map((u, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between p-2.5 rounded-xl border border-gray-50 bg-slate-50/40"
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="flex flex-col text-left">
                      ...
                      <span
                        className="text-[10px] text-gray-400 font-semibold mt-0.5 max-w-[150px] truncate"
                        title={u.department}
                      >
                        {u.department}
                      </span>
              ...
              {filteredAssignedUsers.length === 0 && (
                <p className="text-[11px] text-gray-400 italic text-center py-4">
                  Không tìm thấy tài khoản cùng vai trò
                </p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between">
            <div className="text-left">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">
                Trạng thái vận hành (`status`)
              </span>
              <span
                className={`font-black text-sm tracking-wide mt-1 flex items-center gap-1.5 uppercase font-mono ${
              ...
              >
                <span
                  className={`w-2 h-2 rounded-full ${userInfo.status === "active" ? "bg-emerald-500 animate-pulse" : "bg-amber-400"}`}
                ></span>
                {userInfo.status}
              </span>
            </div>
