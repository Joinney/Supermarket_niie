...
const Quanlytonkho = () => {
  ...
  // State danh sách Nhà cung cấp tự động lấy từ dữ liệu
  ...
  // 1. FETCH VÀ MIX DỮ LIỆU ĐA DỊCH VỤ
  const fetchAllData = async () => {
    setLoading(true);
    try {
      // Chỉ cần gọi 2 API: Inventory (đã chứa sẵn tên NCC) và Product (để lấy Danh mục)
      ...
      const mixedData = rawInventory.map((item) => {
        ...
        return {
          ...
          // 🌟 Lấy trực tiếp Nhà cung cấp từ API Kho trả về (trường supplier_name trong SQL nãy đã làm)
          supplier: item.supplier || "Chưa xác định",
        };
      });

      // Trích xuất danh sách Nhà cung cấp duy nhất để đưa vào Dropdown bộ lọc
      const uniqueSuppliers = [
        ...new Set(mixedData.map((item) => item.supplier)),
      ].filter((s) => s !== "Chưa xác định");
      setGlobalSuppliers(uniqueSuppliers);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, parentCatFilter, childCatFilter, supplierFilter, timeFilter]);
        ...
        <div className="w-full print:hidden">
          ...
          <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex flex-wrap items-center gap-3 mb-6">
            ...
            <select
              ...
              className="px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white text-gray-700 font-bold focus:outline-none w-[160px] cursor-pointer focus:border-[#006c49] truncate"
            >
            ...
            <select
              ...
              className={`px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white text-gray-700 font-bold focus:outline-none w-[160px] truncate ${!parentCatFilter ? "opacity-50 cursor-not-allowed" : "cursor-pointer focus:border-[#006c49]"}`}
            >
            ...
            <select
              ...
              className="px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white text-gray-700 font-bold focus:outline-none w-[170px] cursor-pointer focus:border-[#006c49] truncate"
            >
              <option value="">Tất cả Nhà Cung Cấp</option>
              <option value="Chưa xác định">-- Chưa xác định --</option>
              {globalSuppliers.map((sup) => (
            ...
            <select
              ...
              className="px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white text-gray-700 font-bold focus:outline-none min-w-[180px] focus:border-[#006c49] cursor-pointer"
            >
              <option value="all">Tất cả trạng thái kho</option>
              <option value="inStock">Chỉ lấy hàng còn tồn</option>
              <option value="firstTime">Mới nhập (trong 3 ngày)</option>
              <option value="restocked">Mới update (trong 3 ngày)</option>
              <option value="stagnant">Tồn đọng (&ge; 30 ngày)</option>
            </select>
              ...
              <div className="p-3 rounded-full bg-orange-50 text-orange-500">
                ...
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M12 6v12m-3-2.818l.879-.659c1.171-.879 3.07-.879 4.242 0 1.172.879 1.172 2.303 0 3.182s-3.07.879-4.242 0a1.75 1.75 0 01-.424-.53m0-10.607l.879-.659c1.171-.879 3.07-.879 4.242 0 1.172.879 1.172 2.303 0 3.182s-3.07.879-4.242 0a1.75 1.75 0 01-.424-.53M12 3v3m0 12v3"
                  />
                </svg>
              </div>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-[#f8fafc] border-b border-gray-100 text-[11px] font-bold uppercase tracking-wider text-gray-400">
                    <th className="py-4 px-6 w-[15%]">Mã hàng (SKU)</th>
                    <th className="py-4 px-6 w-[25%]">
                      Tên sản phẩm & Danh mục
                    </th>
                    <th className="py-4 px-6 w-[20%]">Nhà Cung Cấp</th>{" "}
                    {/* 🌟 MỚI */}
                    ...
                    <th className="py-4 px-6 text-right text-[#006c49] w-[15%]">
                      Giá trị dự kiến
                    </th>
                  </tr>
                </thead>
                  ...
                  {loading ? (
                    ...
                    currentTableData.map((item) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-6">
                          <p className="text-gray-800 font-bold max-w-[250px] break-words">
                            {item.name}
                          </p>
                        ...
                        {/* 🌟 HIỂN THỊ CỘT NHÀ CUNG CẤP */}
                        <td className="py-4 px-6">
                          <p
                            className="text-slate-600 font-medium text-xs truncate max-w-[180px]"
                            title={item.supplier}
                          >
                            {item.supplier}
                          </p>
            ...
            <table className="w-full border-collapse border border-black text-sm">
              <thead>
                <tr className="bg-gray-100 font-bold text-center">
                  ...
                  <th className="border border-black px-2 py-2 w-[30%]">
                    Tên Sản Phẩm
                  </th>
                  <th className="border border-black px-2 py-2 w-[15%]">
                    Nhà Cung Cấp
                  </th>{" "}
                  {/* 🌟 MỚI */}
              ...
              <tbody>
                  ...
                  <tr key={item.id}>
                    ...
                    <td className="border border-black px-2 py-2 font-semibold">
                      ...
                      <div className="text-[10px] font-normal italic text-gray-600 mt-1 flex items-center gap-1">
                        ...
                        <svg
                          className="w-2 h-2 text-gray-500 inline"
                          fill="none"
                          stroke="currentColor"
                          strokeWidth="3"
                          viewBox="0 0 24 24"
                    ...
                    <td className="border border-black px-2 py-2 text-center text-xs truncate max-w-[120px]">
                      {item.supplier}
                    </td>{" "}
                    {/* 🌟 MỚI */}
                ...
                <tr className="bg-gray-50 font-bold">
                  ...
                  <td className="border border-black px-2 py-2 text-center text-base">
                    {formatNumber(stats.totalQuantity)}
                  </td>
