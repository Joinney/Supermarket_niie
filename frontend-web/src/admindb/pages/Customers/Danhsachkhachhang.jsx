...
const Danhsachkhachhang = () => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      alert(err.response?.data?.message || "Không thể thực hiện lệnh xóa tài khoản này.");
    } finally {
      setSubmitLoading(false);
    }
  };
  ...
  const formatLastLogin = (dateString) => {
    if (!dateString) return <span className="text-gray-300 italic font-normal text-xs">Chưa đăng nhập</span>;
    const date = new Date(dateString);
    return (
      <div className="font-mono text-xs">
        <p className="text-slate-700 font-bold">{date.toLocaleDateString("vi-VN")}</p>
    ...
    <div className="w-full min-h-screen bg-[#fafafa] font-sans text-gray-800 antialiased p-1 text-left relative">
      ...
      <div className="bg-white p-3.5 rounded-xl border border-gray-100 shadow-sm flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleRefresh} className="px-4 py-2 border border-gray-100 hover:bg-gray-50 bg-white rounded-xl text-xs font-bold text-gray-600 flex items-center gap-2 transition">
            Làm mới
          </button>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden mb-6">
          ...
          <table className="w-full border-collapse text-left text-sm">
            <thead>
              <tr className="bg-[#f8fafc] border-b border-gray-100 text-[11px] font-bold uppercase tracking-wider text-gray-400 select-none">
                ...
                <th className="py-4 px-6 text-center">Lần cuối đăng nhập</th>
            ...
            <tbody className="divide-y divide-gray-50 font-semibold text-slate-600">
              ...
              {!loading && error && (
                <tr>
                  <td colSpan="7" className="py-8 text-center text-red-500 font-bold bg-red-50/50">⚠️ {error}</td>
                </tr>
              )}

              {!loading && !error && customers.length === 0 && (
                <tr>
                  <td colSpan="7" className="py-8 text-center text-gray-400 italic">Không có tài khoản khách hàng Buyer nào.</td>
                </tr>
              )}

              {!loading && !error && customers.map((row) => (
                <tr key={row.user_id} className="hover:bg-slate-50/40 transition-colors">
                  ...
                  <td className="py-4 px-6 text-slate-600 font-medium font-mono text-xs">{row.email}</td>
                  <td className="py-4 px-6 text-slate-600 font-medium font-mono text-xs">
                    {row.phone_number || <span className="text-gray-300 italic font-normal">Trống</span>}
                  </td>
                  
                  <td className="py-4 px-6 text-center text-gray-700 font-medium">
                    {row.gender === "Male" || row.gender === "Nam" ? (
                      <span className="px-2 py-0.5 rounded-md bg-blue-50 text-blue-600 text-xs font-bold">Nam</span>
                    ) : row.gender === "Female" || row.gender === "Nữ" ? (
                      <span className="px-2 py-0.5 rounded-md bg-pink-50 text-pink-600 text-xs font-bold">Nữ</span>
                    ) : (
                      <span className="text-gray-400 font-normal text-xs">{row.gender || "Khác"}</span>
                    )}
                  </td>
        ...
        <div className="p-4 bg-white border-t border-gray-50 flex items-center justify-between text-xs text-gray-400 font-bold flex-wrap gap-4 select-none">
          <div>
            Hiển thị <span className="text-slate-800">{customers.length > 0 ? (currentPage - 1) * limit + 1 : 0}</span> - <span className="text-slate-800">{Math.min(currentPage * limit, totalItems)}</span> trên tổng số <span className="text-slate-800">{totalItems}</span> khách hàng
          </div>
          <div className="flex items-center gap-4 text-gray-500">
            <div className="flex items-center gap-2">
              <span>Trang:</span>
              <select value={currentPage} onChange={(e) => setCurrentPage(Number(e.target.value))} className="border border-gray-200 rounded-xl px-2 py-1 bg-gray-50 text-slate-700 font-bold outline-none cursor-pointer">
                {Array.from({ length: totalPages }, (_, i) => (<option key={i + 1} value={i + 1}>{i + 1}</option>))}
              </select>
            </div>
            <div className="flex items-center gap-1">
              <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="p-1.5 border border-gray-100 rounded-xl disabled:opacity-30">❮</button>
              <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)} className="p-1.5 border border-gray-100 rounded-xl disabled:opacity-30">❯</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-xl max-w-md w-full p-6 animate-scaleUp text-left">
            <div className="flex justify-between items-center pb-3 border-b border-gray-100 mb-4">
              <h3 className="text-base font-bold text-slate-800">📝 Cập nhật thông tin khách hàng</h3>
              <button onClick={() => setEditingUser(null)} className="text-gray-400 hover:text-gray-600 text-lg">✕</button>
            </div>
            
            <form onSubmit={handleSaveEdit} className="space-y-4">
              <div>
                <label className="text-[11px] font-black text-gray-400 uppercase tracking-wider">Họ và tên</label>
                <input 
                  type="text" 
                  required
              ...
              <div>
                <label className="text-[11px] font-black text-gray-400 uppercase tracking-wider">Số điện thoại</label>
                <input 
                  type="text" 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[11px] font-black text-gray-400 uppercase tracking-wider">Giới tính</label>
                  <select 
                ...
                <div>
                  <label className="text-[11px] font-black text-gray-400 uppercase tracking-wider">Trạng thái</label>
                  <select 
      ...
      {deletingUserId && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-xl max-w-sm w-full p-6 animate-scaleUp text-center">
            <div className="w-12 h-12 bg-rose-50 text-rose-500 rounded-full flex items-center justify-center text-xl mx-auto mb-3 font-bold">⚠️</div>
            <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider">Xác nhận xóa tài khoản</h3>
            <p className="text-xs text-gray-500 mt-2 leading-relaxed">
              Bạn có chắc chắn muốn xóa vĩnh viễn tài khoản khách hàng này khỏi hệ thống Demi Mart? Hành động này không thể hoàn tác.
            </p>
