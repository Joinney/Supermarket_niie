...
const ProductCard = ({ p }) => {
  ...
  const country = currentStore?.code?.toLowerCase() || p.country_code?.toLowerCase() || 'vn'; 
      ...
      <div className="w-full group cursor-pointer font-sans bg-white p-2 rounded-[32px] hover:shadow-2xl hover:shadow-slate-100 transition-all duration-500 border border-transparent hover:border-slate-50">
        ...
        <div className="space-y-1 px-1">
          <div className="flex items-baseline gap-2">
            {/* 3. ĐÃ SỬA: Bọc hàm biến đổi giá động tự động tính toán theo tỉ giá quốc gia */}
            <span className="text-[#ff4d4f] font-black text-lg leading-none">
              {formatPrice ? formatPrice(currentPrice) : `${currentPrice.toLocaleString()}đ`}
            </span>
          </div>
          ...
          <div className="flex gap-1 items-center pt-0.5">
            <span className="bg-[#e6f0ed] text-[#006c49] text-[8px] font-black px-1.5 py-0.5 rounded uppercase">{p.ten_danh_muc_con || p.ten_danh_muc || 'Siêu thị'}</span>
          </div>
