...
const RecipeModal = ({ show, onClose, data, onProductClick, onAddComboToCart }) => {
    // Local State quản lý danh sách sản phẩm nguyên liệu kèm số lượng động
    const [selectedProducts, setSelectedProducts] = useState([]);

    // Đồng bộ và khởi tạo dữ liệu khi AI trả về kết quả mới
    useEffect(() => {
        if (data && data.products) {
            const initialized = data.products.map(prod => ({
                ...prod,
                quantity: prod.stock > 0 ? 1 : 0 // Mặc định là 1 nếu còn hàng
            }));
    ...
    // Tăng số lượng sản phẩm chuẩn bị mua
    const handleIncrease = (index) => {
        setSelectedProducts(prev => prev.map((item, idx) => {
            if (idx === index) {
                if (item.quantity >= item.stock) return item; // Giới hạn tối đa theo kho hàng
    ...
    // Giảm số lượng sản phẩm chuẩn bị mua
    ...
    // Hàm phân tích và định dạng chuỗi text thô từ AI thành HTML đẹp mắt
    const formatRecipeText = (text) => {
        ...
        return lines.map((line, index) => {
            ...
            if (trimmed.startsWith('Cách làm') || trimmed.match(/^(Bước\s\d+|Hướng dẫn nhanh|Nguyên liệu có sẵn|Tổng kết)/i)) {
                return (
    ...
    return ReactDOM.createPortal(
        <div className="fixed inset-0 z-[100000] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fadeIn p-4 font-sans">
            <div className="relative w-full max-w-4xl h-[85vh] flex flex-col md:flex-row rounded-3xl bg-white shadow-2xl overflow-hidden border border-gray-100 animate-scaleUp">
                ...
                {/* NÚT ĐÓNG MODAL POPUP VỚI VÒNG ĐỆM TRẮNG NỔI BẬT */}
                <button 
                    onClick={onClose} 
                ...
                {/* BÊN TRÁI: HƯỚNG DẪN NẤU ĂN ĐÃ FORMAT ĐẸP MẮT */}
                <div className="w-full md:w-3/5 p-6 md:p-8 overflow-y-auto border-b md:border-b-0 md:border-r border-gray-100 bg-gradient-to-br from-emerald-50/10 to-white">
                    ...
                    <h2 className="text-xl md:text-2xl font-extrabold text-gray-900 mb-4 capitalize tracking-tight">
                        Cách Làm {data.recipeTitle.replace(/(combo nguyên liệu:|làm|nấu|cần|những|gì|vậy|shop)/gi, '').replace(/\?/g, '').trim()}
                    </h2>
                    <div className="bg-white rounded-2xl p-5 border border border-emerald-100/40 shadow-sm">
                        {formatRecipeText(data.recipeText)}
                    </div>
                </div>

                {/* BÊN PHẢI: KHO HÀNG NGUYÊN LIỆU TÍNH TOÁN SỐ LƯỢNG ĐỘNG */}
                <div className="w-full md:w-2/5 bg-gray-50 flex flex-col h-full overflow-hidden">
                    ...
                    {/* DANH SÁCH THÈ SẢN PHẨM HỖ TRỢ ĐIỀU CHỈNH TĂNG GIẢM XÓA */}
                        ...
                        {selectedProducts.map((prod, index) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {/* Cụm điều khiển hành vi số lượng */}
                                    {isOutStock ? (
                    ...
                    {/* 🌟 FOOTER: CHỈ CÓ 1 NÚT CHỐT ĐƠN DUY NHẤT Ở CUỐI CÙNG TÍNH TỔNG TIỀN */}
                    <div className="p-6 bg-white border-t border-gray-200/80 shadow-[0_-4px_20px_rgba(0,0,0,0.03)]">
                        ...
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="h-4 w-4">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75 childhood.75 0 1 1-1.5 0 .75 .75 0 0 1 1.5 0Zm12.75 0a.75 .75 0 1 1-1.5 0 .75 .75 0 0 1 1.5 0Z" />
                            </svg>
