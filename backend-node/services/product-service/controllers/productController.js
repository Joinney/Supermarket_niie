...
Khi khách hàng hỏi về một món ăn, công thức nấu nướng hoặc nhu cầu tình huống (Ví dụ: "làm bánh mì cần những gì", "muốn làm bánh mì ốp la",...), bạn hãy thực hiện theo đúng 3 bước sau:

Bước 1: Hướng dẫn nhanh công thức hoặc các bước thực hiện món ăn đó một cách ngắn gọn, trực quan, dễ hiểu.
Bước 2: Đối chiếu nghiêm ngặt với DANH SÁCH SẢN PHẨM TRONG KHO dưới đây để chỉ ra những nguyên liệu cụ thể nào đang có sẵn tại Demi Mart giúp khách tiện đặt mua ngay.
Bước 3 (BẮT BUỘC): Ở cuối câu trả lời của bạn, phải đính kèm chính xác mã ID sản phẩm được nhắc đến để tư vấn theo định dạng: [RECOMMEND: id1, id2] (Ví dụ: [RECOMMEND: SP001, SP002]). Nếu khách hỏi chung chung hoặc không có nguyên liệu nào khớp với kho hàng, hãy ghi [RECOMMEND: NONE].
...
${contextStr}`;
        ...
        // 3. XỬ LÝ TRÍCH XUẤT AN TOÀN (Bọc try-catch riêng để không bao giờ bị sập server)
        let finalProducts = [];
        try {
            if (aiReply && aiReply.trim() !== "") {
                ...
                if (match && match[1] && match[1].trim() !== 'NONE') {
                    ...
                    // Cơ chế tìm kiếm nghiêm ngặt bằng từ khóa cốt lõi
            ...
            // Nếu lỗi trích xuất, giữ mảng trống chứ không làm sập luồng chính
            finalProducts = [];
        }
